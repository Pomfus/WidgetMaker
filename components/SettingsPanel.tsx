...
export const PresetsPanel: React.FC<SharedProps> = ({ settings, setSettings }) => {
  ...
  return (
    <div className="bg-slate-900/95 text-white p-6 rounded-2xl shadow-2xl border-2 border-slate-700 w-full h-full flex flex-col overflow-hidden">
      <h2 className="text-xl font-black mb-6 flex items-center gap-2 tracking-tighter italic shrink-0">
        <span className="text-cyan-500">AESTHETIC</span> PRESETS
      </h2>
      <div className="space-y-6 overflow-y-auto pr-2 custom-scrollbar flex-1">
        {Object.entries(themeCategories).map(([cat, themes]) => (
          <div key={cat} className="border-b border-slate-800 pb-4 last:border-0">
            <h4 className="text-[10px] text-cyan-500/70 mb-3 font-black uppercase tracking-widest">{cat}</h4>
...
interface ControlsPanelProps extends SharedProps {
  tiers: string[];
  onExport: () => void;
  hasMembers: boolean;
...
export const ControlsPanel: React.FC<ControlsPanelProps> = ({ 
  ...
  return (
    <div className="bg-slate-900/95 text-white p-6 rounded-2xl shadow-2xl border-2 border-slate-700 w-full h-full overflow-y-auto custom-scrollbar">
      <h2 className="text-xl font-black mb-6 flex items-center gap-2 tracking-tighter italic">
        <span className="text-cyan-500">TACTICAL</span> CONTROLS
      </h2>
      ...
      <div className="space-y-8">
              ...
              <button
                ...
                className={`px-3 py-2 rounded text-[9px] font-black transition-all border text-left ${
                  settings.sortOrder === opt.id 
                    ? 'bg-cyan-500 text-black border-cyan-300 shadow-[0_0_10px_rgba(6,182,212,0.3)]' 
                    : 'bg-slate-800 text-slate-400 border-slate-700 hover:border-slate-500'
                }`}
              >
        ...
        <section className="bg-slate-800/50 p-4 rounded-xl border border-slate-700">
          <label className="block text-[10px] font-black mb-3 uppercase tracking-[0.2em] text-cyan-400">Broadcast Announcement</label>
          <textarea
            value={settings.customMessage}
            onChange={(e) => updateSetting('customMessage', e.target.value)}
            placeholder="Type your custom announcement here..."
            className="w-full h-16 bg-slate-950 border border-slate-700 rounded-lg p-3 text-xs font-medium focus:ring-1 focus:ring-cyan-500 outline-none transition-all placeholder:text-slate-700 resize-none"
          />
        ...
        <section className="bg-slate-800/50 p-4 rounded-lg border border-slate-700">
           <label className="block text-[10px] font-black mb-3 uppercase tracking-[0.2em] text-slate-500">Data Visibility</label>
        ...
        <section className="space-y-6">
          <label className="block text-[10px] font-black mb-1 uppercase tracking-[0.2em] text-cyan-500">Hull Dynamics</label>
        ...
        <section className="bg-slate-800/80 p-5 rounded-xl border border-slate-700 shadow-inner">
          <label className="block text-[10px] font-black mb-3 uppercase tracking-[0.2em] text-cyan-400">Archive Manager</label>
          <div className="flex gap-2 mb-4">
            <input 
              ...
              className="flex-1 bg-slate-950 border border-slate-700 rounded-lg p-3 text-xs font-medium focus:ring-1 focus:ring-cyan-500 outline-none transition-all placeholder:text-slate-700"
            />
            <button 
              onClick={handleSave}
            ...
            {favorites.map(fav => (
              <div key={fav.id} className="bg-slate-900 p-3 rounded border border-slate-800 flex items-center justify-between group">
                ...
                <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => onLoad(fav.id)} className="text-[9px] font-black text-cyan-500 uppercase">Load</button>
                  <button onClick={() => onDelete(fav.id)} className="text-[9px] font-black text-red-500 uppercase">Scrap</button>
                </div>
              </div>
            ))}
          </div>
        ...
        {hasMembers && (
          <button
            onClick={onExport}
            className="w-full bg-cyan-600 hover:bg-cyan-500 text-black font-black py-5 rounded-xl flex items-center justify-center gap-2 transition-all active:scale-95 shadow-xl uppercase tracking-tighter italic text-lg"
          >
